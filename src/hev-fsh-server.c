...
hev_fsh_server_task_entry (void *data)
{
	...
	for (;;) {
		...
		session = hev_fsh_server_session_new (client_fd, session_close_handler, self);
		if (!session) {
			close (client_fd);
			continue;
		}
...
static void
session_manager_insert_session (HevFshServer *self, HevFshServerSession *session)
{
...
static void
session_manager_remove_session (HevFshServer *self, HevFshServerSession *session)
{
	HevFshServerSessionBase *session_base = (HevFshServerSessionBase *) session;
